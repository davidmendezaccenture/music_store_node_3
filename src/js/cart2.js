...
//Declaramos variables para dar la opción de paginar los elementos
let paginaActual = 1;
const itemsPorPagina=2;
let modoPaginacion=false;
...
$(document).ready(function () {
  ...
  actualizarTextoBotonPaginacion();
});
...
function mostrarCarrito() {
  ...
  let totalCarrito = 0;
//Si estamos en modo paginacion, creamos las páginas que vamos a usar con los elementos definidos, si no mostramos carrito completo
  let itemsMostrar;
    if(modoPaginacion) {
      const inicio = (paginaActual - 1) * itemsPorPagina;
      const fin = inicio + itemsPorPagina;
      itemsMostrar = carrito.slice(inicio, fin);
    } else {
      itemsMostrar = carrito;
  }
...
`;
  ...
  //Para sumar el total del carrito aunque tengamos varias páginas
    totalCarrito = carrito.reduce((acum, item) => {
    const producto = productosDisponibles.find(p => p.id === item.id);
    return producto ? acum + producto.offerPrice * item.cantidad : acum;
  }, 0);
  ...
  if(modoPaginacion) {
    mostrarPaginacion();
      ...
      $('#paginacion-carrito').empty(); // Quita paginación si está visible
  }
...
function mostrarPaginacion() {
  const $paginacion = $('#paginacion-carrito');
  $paginacion.empty();

  const totalPaginas = Math.ceil(carrito.length / itemsPorPagina);

  if (totalPaginas <= 1) return; // No mostramos si solo hay 1 página

  for(let i = 1; i <= totalPaginas; i++) {
    const boton = $('<button></button>')
      .addClass('btn btn-sm mx-1')
      .text(i)
      .toggleClass('btn-primary', i === paginaActual)
      .toggleClass('btn-outline-primary', i !== paginaActual)
      .click(() => {
        paginaActual = i;
        mostrarCarrito();
      });
...
//Función que controla el botón para mostrar todos los elementos o solo 3

$('#toggle-paginacion').click(() => {
  modoPaginacion = !modoPaginacion;
  paginaActual = 1; // reset paginación a página 1 al cambiar modo
  mostrarCarrito();
  actualizarTextoBotonPaginacion();
...
function actualizarTextoBotonPaginacion() {
  if(modoPaginacion) {
    $('#toggle-paginacion').text('Mostrar todo');
  } else {
    $('#toggle-paginacion').text('Mostrar ' + itemsPorPagina + ' por página');
  }
}

//Archivo con paginación